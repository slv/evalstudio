...
export async function personasRoute(fastify: FastifyInstance) {
  fastify.get("/personas", async (request) => {
    const { personas } = createProjectModules(fastify.storage, request.projectCtx!.id, fastify.connectorRegistry);
  ...
  fastify.post<{ Params: PersonaParams }>(
    ...
    async (request, reply) => {
      ...
      const { personas } = createProjectModules(fastify.storage, ctx.id, fastify.connectorRegistry);
      const persona = await personas.get(request.params.id);
